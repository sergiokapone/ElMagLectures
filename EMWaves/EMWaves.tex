...
\begin{frame}{Плоска хвиля}
	\begin{columns}
		\begin{column}{0.6\linewidth}
			\begin{block}{}\justifying
				\alert{Хвиля називається плоскою}, якщо $u$ має одну й ту саму величину в усіх точках будь-якої площини, що
				перпендикулярна до напрямку поширення хвилі. Від площини до площини $u$, звісно, змінюється. \alert{Поверхнями постійної фази} в плоскій хвилі є
				площини,
	...
	\begin{overprint}\small
		...
		\begin{block}{}\justifying
			...
			$\lambda = \frac{2\pi}{k}$ називається \alert{довжиною хвилі}, $\vect{k}$ --- називається \alert{хвильовим вектором}, який напрямлений у напрямку
			поширення
...
\begin{frame}{Формули Френеля для $s$-поляризації}
	\only<1>{Граничні умови для вектора $\Efield$ дають:
		...
		\begin{equation*}
			\tcbhighmath{k_x E_0 + k'_x E'_0 = k''_x E''_0}
		\end{equation*}
	}
	\only<2>{
		\[
		...
		\]
		\[
			k_xE_0 + k'_xE'_0 = k''_xE''_0
		\]
		\[
			k_x = -k'_x =k\cos i, \quad k'' = n_{21} k, \quad k''_x = k''\cos r = n_{21} k\cos r
		\]
		\[
			E_{0} \cos i - E'_0 \cos i = n_{21} E''_0 \cos r
		\]
		\[
			E_0 \cos i - E'_0 \cos i = n_{21} (E_0 + E'_0) \cos r
		\]
		...
		\begin{column}{0.5\linewidth}
			\only<1>{
				...
				\begin{equation*}
					E_y + E'_{y_0} = E''_{y_0}
				\end{equation*}
				З рівності  $E_{y_0} = \frac{c k_x}{\epsilon \omega} B$ друга умова приймає вигляд:
				\begin{equation*}
					\frac{k_x}{\epsilon_1}\left( B_0 - B'_0\right)  = \frac{k''_x}{\epsilon_2} B''_0
				\end{equation*}
				або
				\begin{equation*}
					n_{21} \cos i \left( B_0 - B'_0\right)  = \cos r B''_0
				\end{equation*}
			}
			\only<2>{
...
\begin{frame}{Повне внутрішнє відбивання}
	...
	\only<2>{
	\[
		\Efield'' = \Efield''_0 e^{i(\omega t - {k''_x} - {k''_y})}
	\]
	\[
