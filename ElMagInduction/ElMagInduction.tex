			...
			\begin{block}{}\centering\itshape\small
				\tikz[pencildraw/.style={ %
					...
					\node[align=center,
						...
						pencildraw,draw,fill=white,text width = \linewidth,inner sep=2mm, drop shadow]
					{Восени 1831 року Майкл Фарадей виконав завдання, яке ще в 1822 року записав собі в пам'ятній книжці,
						\alert{<<перетворити магнетизм на електрику>>}.
					}
				}
			\end{block}
...
\begin{frame}{Закон електромагнітної індукції}{}
	...
	\begin{block}{Закон Електромагнітної індукції}\justifying
		...
		\begin{equation*}
			\tcbhighmath{\mathcal{E}_\text{ind} =  -\frac1c \frac{d\Phi}{dt}} = -\frac1c \frac{d}{dt} \iint\limits_S \Bfield\cdot d\vect{S}.
		\end{equation*}
	\end{block}
	\begin{block}{}
...
\begin{frame}{Правило Ленца}{}
	\begin{block}{Правило Ленца}\justifying
		\textcolor{red}{Індукційний струм} має такий напрямок, щоб за допомогою \textcolor{cyan}{створюваного ним магнітного поля} перешкоджати зміні
		\textcolor{blue}{магнітного потоку}, тобто щоб послабити причину, яка збуджує цей струм.
	\end{block}
	\begin{columns}
...
% ============================== Слайд ## ===================================
\begin{frame}{Закон збереження магнітного потоку}{}\small
	\begin{block}{}
			...
			\begin{tikzpicture}[>=latex, scale=0.6, transform shape]
				\begin{scope}
					...
					\foreach[count=\i from -2] \x in {-0.8,-0.4,...,1} {
							\draw[->, blue, thick] (\x, -2) -- ++(0,4) \ifnum\i=5 node[right] {$\Bfield$}\fi;
						}
...
\begin{frame}{Вихрове електричне поле}{}
	...
	%		anchor=east] at ([yshift=1.5cm]current page.east){\includegraphics[width=2cm]{Maxwell}};
	\begin{onlyenv}<1>
		\begin{block}{}\justifying
			...
			\begin{equation*}
				\tcbhighmath{\oint\limits_L \Efield\cdot d\vect{\ell} = - \frac1{c} \iint\limits_S  \frac{\partial \Bfield}{\partial t}\cdot d\vect{S}.}
			\end{equation*}
		\end{block}
		\begin{block}{Теорема про циркуляцію}\justifying
			Циркуляція напруженості вихрового електричного поля по довільному контуру дорівнює взятому з протилежним знаком потокові швидкості зміни
			індукції магнітного поля через довільну поверхню обмежену цим контуром.
		\end{block}
	\end{onlyenv}
	\begin{onlyenv}<1-2>
		\begin{block}{}\justifying
			\only<1>{Скориставшись теоремою Стокса, останнє інтегральне рівняння можна переписати у диференціальній формі:}
			\begin{equation*}
...
% ============================== Слайд ## ===================================
\begin{frame}[t]{Чому вихрове поле --- електричне?}{}
	\tikz[remember picture,overlay] \node[opacity=0.75,inner sep=0pt,
		anchor=south west] at (current page.south west){\includegraphics[width=2cm]{Grigorev}};
	\begin{block}{}\centering\itshape\small
		\tikz[pencildraw/.style={ %
					decorate,
					decoration={random steps,segment length=2pt,amplitude=1pt}
			...
			\node[align=justify,
				...
				pencildraw,draw,fill=white,text width = \linewidth,inner sep=2mm, drop shadow]
			{
				\alert{Електричне поле, що виникає при зміні магнітного поля, має зовсім іншу структуру, ніж електростатичне}. Воно не пов'язане безпосередньо з
				електричними зарядами, і його силові лінії не можуть на них починатися і закінчуватися. Вони взагалі ніде не починаються і ніде не закінчуються, являючи
				собою замкнуті лінії, подібні до силових ліній магнітного поля. Це так зване вихрове поле.

				\bigskip

				При зміні поля сильного електромагніту з'являються потужні вихори електричного поля, які можна використовувати для прискорення електронів до швидкостей,
				близьких до швидкості світла. На цьому принципі ґрунтується пристрій прискорювача електронів --- бетатрона. Електричний струм у бетатроні виникає
				безпосередньо у вакуумній камері без введення будь-яких металевих провідників. Може виникнути запитання: \alert{а чому, власне, це поле називається
					електричним}? Адже воно має інше походження та іншу конфігурацію, ніж статичне електричне поле. Відповідь проста: \alert{вихрове
					поле діє на заряд точно так само, як і електростатичне, а це ми вважали і вважаємо головною властивістю поля}.
			}
		}
	\end{block}
\end{frame}
...
\begin{frame}{Явище самоіндукції}{}
		...
		\begin{column}{0.64\linewidth}
			\begin{block}{}\justifying
				...
				\begin{equation*}
					\Phi = \frac1c L I
				\end{equation*}
				$L$ --- називається \alert{індуктивністю контуру}.
			\end{block}
		\end{column}
	\end{columns}
	\begin{block}{}\justifying
	...
	\begin{alertblock}{}\justifying\scriptsize
		Знак мінус показує, що $\mathcal{E}$ завжди спрямована так, щоб перешкоджати зміні сили струму відповідно до
...
\begin{frame}{Теорема взаємності}{}
    \begin{block}{Теорема взаємності}\justifying
        ЕРС, що наводиться в контурі $1$, коли струм в контурі $2$ змінюється з певною швидкістю, дорівнює ЕРС, яка наводилася б в контурі $2$, якби той самий змінний струм протікав в контурі $1$:
            \[\mathcal{E}_1  = -L_{12}\frac{d{\color{red}I}}{dt} {\color{red}=}  \mathcal{E}_2  = -L_{21}\frac{d{\color{red}I}}{dt}\]
    \end{block}
	\begin{block}{Наслідок}\justifying
		Коефіцієнтами взаємної індуктивності контурів однакові:
		\begin{equation*}
	...
	Якщо по контурам течуть однакові струми $I$, то
		\begin{equation*}
			\Phi_1 = \Phi_2
		\end{equation*}
		{\scriptsize Це дає змогу спрощувати задачі.}
	\end{block}
\end{frame}
% ===========================================================================
...
% ============================== Слайд ## ===================================
\begin{frame}{Задачі на застосування теореми взаємності}{}\small
...
\begin{frame}{Пондеромоторні сили в магнітному полі}{}
	...
	\begin{overprint}
		...
		\begin{block}{}\justifying
			...
			\end{equation*}
			З іншого боку, та ж робота визначається як (закон Ома):
			\begin{equation*}
...
\begin{frame}{Приклади}{Поведінка різних тіл в магнітному полі}
	\begin{tblr}{
		...
		\begin{pict}\includegraphics[width=0.5\linewidth]{amplula_FeCl}\end{pict} &
		\begin{pict}\includegraphics[width=0.5\linewidth]{probirka_FeCl}\end{pict}                                                       \\
		Ампула з парамагнітним розчином хлористого заліза в магнітному полі       & Втягування розчину хлористого заліза в магнітне поле \\
		\begin{pict}\includegraphics[width=0.5\linewidth]{palochka_Bi}\end{pict}  &
