...
%            }
	...
	\end{columns}
	\begin{block}{}\justifying
		Напруженість \alert{електричного поля хвилі \( E_\perp \) спадає як \( {1}/{r} \)},
		...
		а густина енергії \( \propto E^2 \).
		Крім того, \alert{\( E_\perp \) у момент часу \( t \) залежить від прискорення заряду \( a \)
...
\begin{pict}
		\begin{tikzpicture}
			\draw[thick, red, double, -stealth] (0,-0.5) node[right, font=\scriptsize] {диполь в момент часу $ t' =  t - \frac{r}{c} $} -- ++(0,1)
			node[left] {$\vect{p}_0$};
			\draw[-latex] (0,0) -- node[anchor=south east] {$\vect{r}$} node[below, sloped] {$ \Delta t = r/c $} (25:4) node[right, font=\scriptsize]
			{Поля в момент часу $ t $};
		\end{tikzpicture}
\end{pict}
	\end{center}
...
%	\end{block}
	...
	\begin{minipage}{0.6\linewidth}\centering
		\begin{align*}\small
			B_{\phi}   & = \frac{i\omega}{c}\sin\theta\left( \frac1r + \frac{i\omega}{c}\right) \cdot \frac{\vect{p}_0 e^{i\omega (t - r/c)}}{r},                \\
			E_r        & = 2\cos\theta\left( \frac{1}{r^2} + \frac{i\omega}{cr}\right) \cdot \frac{\vect{p}_0 e^{i\omega (t - r/c)}}{r},                         \\
			E_{\theta} & = \sin\theta \left( \frac{1}{r^2} + \frac{i\omega}{cr} - \frac{\omega^2}{c^2} \right) \cdot \frac{\vect{p}_0 e^{i\omega (t - r/c)}}{r}.
		\end{align*}
	\end{minipage}%
	%---------------------------------------------------------
...
\begin{frame}{Поля диполя}{Хвильова зона}
	\begin{block}{}
		...
		\end{equation*}
		називається \alert{хвильовою зоною}.
	\end{block}
	\begin{columns}
			...
			\begin{overprint}
				...
				\begin{center}
					\begin{pict}
						...
						\pgfmathsetmacro{\phivec}{60}
						%
						\begin{tikzpicture}[scale=2.0,tdplot_main_coords, >=latex]
							\coordinate (O) at (0,0,0);
							\draw[thick,->] (0,0,0) -- (1,0,0) node[anchor=north east]{$x$};
							\draw[thick,->] (0,0,0) -- (0,1,0) node[anchor=north west]{$y$};
							\draw[thick,->] (0,0,0) -- (0,0,1) node[anchor=south]{$z$};
				...
				\begin{center}
					\begin{pict}
						\begin{tikzpicture}[>=stealth]
							\draw[thick,color=blue,domain=0:2*pi,samples=200,smooth, name path=leaf] plot (xy polar cs:angle=\x r,radius= {1.5*abs(cos(\x
									r))});
							...
							\node[text width=2cm, font=\tiny, align=center, inner sep=0, gray] (TEXT) at (0.5,-1.6) {Довжина відрізка\\ дорівнює\\ величині
								поля};
...
\begin{frame}{Потужність, що випромінюється осцилятором}{}
	\begin{block}{}\justifying
		Густина потоку електромагнітної енергії характеризується вектором Пойнтінга $ \vect{\Pi} $. Тому потік електромагнітної, енергії крізь поверхню сфери радіусом $ r $, що оточує осцилятор, називається \emph{потужністю випромінювання}.
	\end{block}
	\vspace*{-1em}
	\begin{overprint}
		\onslide<1>
		...
		\onslide<2>
		\begin{multline*}
			\frac{dW}{dt} = -\oiint\limits_{\sigma} \vect{\Pi}d \vect{\sigma} =  -\int\limits_0^{2\pi}\int\limits_0^{\pi} \frac{c}{4\pi}|[\Efield\times\Bfield]| \, r^2\sin\theta d\theta d\phi = \\
		...
		\onslide<3>
		\begin{alertblock}{}\small\justifying
			Осцилятор неперервно випромінює енергію в навколишній простір, причому, середня швидкість випромінювання енергії
			\begin{equation*}
				\left\langle \frac{d W}{dt} \right\rangle = \frac1T\int\limits_0^T \frac{dW}{dt} dt = \frac{\omega^4p_0^2}{3c^3}
			\end{equation*}
        ...
        \begin{block}{}\small\justifying
            Для радіозв’язку використовують відносно короткі хвилі
            (десятки метрів --- кілометри), оскільки довгі хвилі від
            повільно змінних струмів практично не випромінюються.
        \end{block}
...
\begin{frame}[t]{Задачі}{}
	\begin{exampleblock}{Задача 1}\justifying
		Електромагнітна хвиля, що випромінюється диполем, поширюється у вакуумі так, що у хвильовій зоні на промені, перпендикулярному до осі диполя, на відстані $ r $ від нього середнє значення густини потоку енергії дорівнює $ \left\langle \Pi \right\rangle $. Знайти середню потужність випромінювання диполя.
	\end{exampleblock}
