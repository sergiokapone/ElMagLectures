...
\begin{frame}[t]{Цитати із книги <<Еволюція фізики>>}{А. Ейнштейн, Л. Інфельд}
	\tikz[remember picture,overlay] \node[opacity=1,inner sep=0pt,
		anchor=north west] at (current page.north west){\includegraphics[width=1.2cm]{Evolution}};
...
\begin{frame}{Закон збереження енергії електромагнітного поля}{}
	\begin{columns}
		...
		\begin{column}{0.59\linewidth}
			\begin{block}{}\justifying
				Якщо  у деякій області простору присутнє \alert{електромагнітне поле та заряджені частинки, які взаємодіють з цим полем}, то енергія
				такої системи має
				зберігатись, якщо система замкнена. Якщо ж система незамкнена, то \alert{енергія може як втікати в цю область, так і витікати з неї}.
			\end{block}
		\end{column}
	\end{columns}
...
\begin{frame}{Скін-ефект}{Якісне пояснення}
	...
	\begin{overprint}
		...
		\begin{columns}
			\begin{column}{0.4\linewidth}\centering
				\input{tikz/skin-effect.tikz}
			\end{column}
			\begin{column}{0.6\linewidth}
				\begin{block}{}\justifying\small
					Змінний струм у провіднику породжує змінне \textcolor{blue}{вихрове магнітне поле}, силові лінії якого перпендикулярні до осі
					провідника. Змінне
					магнітне поле породжує \textcolor{red}{вихрове електричне поле}, що спричиняє виникнення струмів Фуко,
					причому на поверхні провідника вихровий
					струм спрямований у напрямку струму провідника, а всередині провідника --- протилежно. Це явище знижує струм у серцевині провідника і
					збільшує
					поверхневий струм.
				\end{block}
			\end{column}
		...
		\begin{block}{}\justifying\small
			...
			\begin{itemize}\justifying
                ...
                Оскільки за наявності сильного скін-ефекту, струму, а отже і магнітного поля всередині
				провідника немає. Магнітна енергія стає меншою на величину енергії поля всередині провідника, а отже, \alert{індуктивність провідника
				зменшується}.
			\end{itemize}
		\end{block}
	\end{overprint}
...
\begin{frame}{Скін-ефект}{Рівняння скін-ефекту}
	\framesubtitle<1>{Чому у металах нехтують струмами зміщення?}
	\framesubtitle<2>{Рівняння скін-ефекту}
	\framesubtitle<3>{Глибина проникнення поля (струму) в провідник}
	\begin{onlyenv}<1>
		\begin{block}{}\justifying
			У провіднику густина струму провідності і напруженість електричного поля пов'язані законом Ома: $
				\vect{j} = \lambda\Efield. $
			Якщо електричне поле в провіднику змінюється з часом, наприклад, за гармонічним законом $ \Efield=\Efield_0\cos(\omega t)$,
			то поряд зі струмом провідності виникне ще й струм зміщення:
			\begin{equation*}
				\vect{j}_\text{зм} = \frac{\epsilon}{4\pi} \parttime{\Efield} = \frac{\epsilon\omega}{4\pi} E_0\sin(\omega t + \pi/2).
			\end{equation*}
			Де величину $\frac{\epsilon\omega}{4\pi}$ можна формально трактувати як <<питому провідність>> для струмів зміщення.
	...
	\begin{onlyenv}<2>\small
		\begin{block}{}\justifying
			...
			У цих припущеннях запишемо рівняння Максвелла для
	...
	\begin{onlyenv}<3>
		\begin{block}{}\justifying\small
			...
			$\tau \approx T$.
			Нарешті, маючи на увазі, що $T = 2\pi/\omega$, отримуємо для глибини проникнення
			\begin{equation*}
				\delta =\frac{c}{\sqrt{2\lambda\mu\omega}}.
			\end{equation*}
		\end{block}

		\begin{columns}
			\begin{column}{0.5\linewidth}\scriptsize
				\begin{tblr}{
					colspec= {Q[c ,m]|Q[c,m]},
					hlines, vlines,
					}
					{Метал}       & {Глибина \\ скін-шару (мкм)\\ для $10$~ГГц} \\
					Мідь (Cu)     & 0.652    \\
					Срібло (Ag)   & 0.634    \\
					Золото (Au)   & 0.753    \\
					Алюміній (Al) & 0.652    \\
			...
			\begin{column}{0.5\linewidth}\scriptsize
				...
				\begin{tikzpicture}[]
					\begin{loglogaxis}[
							trim axis right,
							title={Глибина скін-шару міді в залежності від частоти},
							xlabel={Частота (Гц)},
							ylabel={Глибина (мкм)},
							grid=both,
					...
					\end{loglogaxis}
					\node[anchor=north] at (2, -0.5) {\includegraphics[width=0.75\linewidth]{skin2}};
				\end{tikzpicture}
			\end{column}
		\end{columns}
	\end{onlyenv}
