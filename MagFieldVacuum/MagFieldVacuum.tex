...
\begin{frame}[plain]
	...
	%		anchor=north west] at (current page.
...
\begin{frame}{Елемент струму}{}
	\begin{onlyenv}<1>
		\begin{columns}
			\begin{column}{0.3\linewidth}\centering
				\input{tikz/CurrentElementSpy.tikz}
			\end{column}
	...
	\begin{onlyenv}<2>
		\begin{tblr}{
			...
			\input{tikz/VolCurrentElement.tikz}
			 &
			Елемент об'ємного струму $\vect{j}dV$
			\\
			\input{tikz/LinCurrentElement.tikz}
			 &
			Елемент лінійного струму $Id\vect{\ell}$
			\\
			\input{tikz/SurfCurrentElement.tikz}
			 &
			{
...
% ============================== Слайд ## ===================================
\begin{frame}{Взаємодія струмів}{Досліди Ампера}
	\begin{block}{}\justifying
...
\begin{frame}{Теорема про циркуляцію магнітного поля у вакуумі}{Інтегральна форма}
	...
	\begin{columns}
		...
		\begin{tikzpicture}[>=latex]
			...
			\end{scope}
            \node[rotate=90] at (-1.5, 0) {\includegraphics[width=1cm]{corksckrew.png}};
		\end{tikzpicture}
		\end{column}
	\end{columns}
\end{frame}
